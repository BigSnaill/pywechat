...
class Messages():
    ...
    @staticmethod
    def send_message_to_friend(friend: str, message: str, at: list[str] = [], at_all: bool = False,
                               tickle: bool = False, search_pages: int = 5, wechat_path: str = None,
        ...
        edit_area, main_window = Tools.open_dialog_window(friend=friend, wechat_path=wechat_path,
                                                          is_maximize=is_maximize, search_pages=search_pages)
        ...
        if at_all:
            ...
            if chatContactMenu.exists():
                ...
                if groupMemberList.children()[0].window_text() == ListItems.MentionAllListItem['title'] and \
                        groupMemberList.children()[1].window_text() == '':
    ...
    @staticmethod
    def send_messages_to_friend(friend: str, messages: list[str], tickle: bool = False, delay: float = 0.4,
                                search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                                close_wechat: bool = True) -> None:
        '''
        该方法用于给单个好友或群聊发送多条信息
        Args:
            friend:好友或群聊备注。格式:friend="好友或群聊备注"
    ...
    @staticmethod
    def send_messages_to_friends(friends: list[str], messages: list[list[str]], tickle: list[bool] = [],
                                 delay: float = 0.4, wechat_path: str = None, is_maximize: bool = True,
                                 close_wechat: bool = True) -> None:
        '''
        该方法用于给多个好友或群聊发送多条信息
        Args:
        ...
        i = 0
        for friend in Chats:
            edit_area, main_window = Tools.open_dialog_window(friend=friend, search_pages=0, close_wechat=False,
                                                              wechat_path=wechat_path, is_maximize=is_maximize)
            edit_area.click_input()
    ...
    @staticmethod
    def send_message_to_friends(friends: list[str], message: list[str], tickle: list[bool] = [], delay: float = 0.4,
                                wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        ...
        i = 0
        for friend in Chats:
            edit_area, main_window = Tools.open_dialog_window(friend=friend, search_pages=0, wechat_path=wechat_path,
    ...
    @staticmethod
    def forward_message(friends: list[str], message: str, delay: float = 0.5, search_pages: int = 5,
                        wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        ...
        def right_click_message():
            ...
            chats = [chat for chat in chatList.children(control_type='ListItem') if
                     chat.descendants(control_type='Button')]  # 不包括时间戳和系统消息
            sent_message = [sent_message for sent_message in chats if
                            sent_message.descendants(control_type='Button')[-1].window_text() == myname]
            while not sent_message and sent_message[
                -1].is_visible():  # 如果聊天记录页为空或者我发过的最后一条消息(转发的消息)不可见(可能是网络延迟发送较慢造成，也不排除是被顶到最上边了)
                chats = [chat for chat in chatList.children(control_type='ListItem') if
                         chat.descendants(control_type='Button')]  # 不包括时间戳和系统消息,只有是好友发送的时候，这个消息内部才有(Button属性)
                sent_message = [sent_message for sent_message in chats if
                                sent_message.descendants(control_type='Button')[-1].window_text() == myname]
        ...
        if len(message) == 0:
            raise CantSendEmptyMessageError
        edit_area, main_window = Tools.open_dialog_window(friends[0], wechat_path, is_maximize=is_maximize,
    ...
    @staticmethod
    def check_new_message(duration: str = None, save_file: bool = False, target_folder: str = None,
                          wechat_path: str = None, close_wechat: bool = True) -> list[dict]:
        '''
        ...
        def get_message_content(name, number, search_pages):
            ...
            if voice_call_button.exists() and not video_call_button.exists():  # 只有语音和聊天按钮没有视频聊天按钮是群聊
                ...
                for ListItem in ListItems:
                    message_sender, message_content, message_type = Tools.parse_message_content(ListItem=ListItem,
                ...
                return {'好友名称': name, '好友类型': friendtype, '新消息条数': number, '消息内容': message_contents,
                        '消息类型': message_types, '发送消息群成员': message_senders}
        ...
        if not duration:  # 没有持续时间。
            ...
            if not chatsButton.legacy_properties().get(
                    'Value'):  # 通过微信侧边栏的聊天按钮的LegarcyProperties.value有没有消息提示(右上角的红色数字,有的话这个值为'\d+')
                if close_wechat:
                    main_window.close()
                print('未查找到新消息')
                return None
            ...
            if not scrollable:  # 聊天列表不足12人以上,会话列表没有满,没有滑块，不用遍历,直接原地等待即可
                ...
                while time.time() < end_timestamp:  # 一直等截止时间
                    break
                if not chatsButton.legacy_properties().get(
                        'Value'):  # 文件传输助手界面原地等待duration后如果侧边栏的消息按钮还是没有红色消息提示直接返回未查找到新消息
                    if close_wechat:
                        main_window.close()
                    print('未查找到新消息')
                    return None
...
class Files():
    ...
    @staticmethod
    def send_file_to_friend(friend: str, file_path: str, at: list[str] = [], at_all: bool = False,
                            with_messages: bool = False, messages: list = [], messages_first: bool = False,
                            delay: float = 0.4, tickle: bool = False, search_pages: int = 5, wechat_path: str = None,
                            is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        该方法用于给单个好友或群聊发送单个文件
        Args:
    ...
    @staticmethod
    def send_files_to_friend(friend: str, folder_path: str, with_messages: bool = False, messages: list = [str],
                             messages_first: bool = False, delay: float = 0.4, tickle: bool = False,
                             wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True,
                             search_pages: int = 5) -> None:
        '''
        该方法用于给单个好友或群聊发送多个文件
        Args:
            friend:好友或群聊备注。格式:friend="好友或群聊备注"
            ...
            if len(files_in_folder) <= 9:
                ...
                for i in range(0, files_num, 9):
                    if i + 9 < files_num:
                        ...
                        pyautogui.hotkey('alt', 's', _pause=False)
                if rem:
                    Systemsettings.copy_files_to_windowsclipboard(
                        filepaths_list=files_in_folder[files_num - rem:files_num])
                    pyautogui.hotkey("ctrl", "v")
                    time.sleep(delay)
                    pyautogui.hotkey('alt', 's', _pause=False)
    ...
    @staticmethod
    def send_file_to_friends(friends: list[str], file_paths: list[str], with_messages: bool = False,
                             messages: list[list[str]] = [], messages_first: bool = False, delay: float = 0.4,
                             tickle: list[bool] = [], wechat_path: str = None, is_maximize: bool = True,
                             close_wechat: bool = True) -> None:
        '''
        ...
        if with_messages and messages:
            ...
            i = 0
            for friend in Files:
                edit_area, main_window = Tools.open_dialog_window(friend=friend, search_pages=0,
                                                                  wechat_path=wechat_path, is_maximize=is_maximize)
                edit_area.click_input()
                if messages_first:
    ...
    @staticmethod
    def send_files_to_friends(friends: list[str], folder_paths: list[str], with_messages: bool = False,
                              messages: list[list[str]] = [], messages_first: bool = False, delay: float = 0.4,
                              tickle: list[bool] = [], wechat_path: str = None, is_maximize: bool = True,
                              close_wechat: bool = True) -> None:
        '''
    ...
    @staticmethod
    def forward_file(friends: list[str], file_path: str, delay: float = 0.4, search_pages: int = 5,
                     wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        ...
        edit_area, main_window = Tools.open_dialog_window(friend=friends[0], wechat_path=wechat_path,
                                                          is_maximize=is_maximize, search_pages=search_pages)
        edit_area.click_input()
        Systemsettings.copy_file_to_windowsclipboard(file_path=file_path)
        pyautogui.hotkey("ctrl", "v")
    ...
    @staticmethod
    def save_files(friend: str, folder_path: str = os.getcwd(), number: int = 10, search_pages: int = 5,
                   wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> list[str]:
        '''
        ...
        chat_history_window, main_window = Tools.open_chat_history(friend=friend, TabItem='文件', wechat_path=wechat_path,
                                                                   close_wechat=close_wechat, is_maximize=is_maximize,
                                                                   search_pages=search_pages)
        ...
        for filepath, count in repeat_counts.items():
            if count > 1:  # 重复次数大于1
                ...
                samefilepaths = [os.path.join(folder, file) for file in os.listdir(folder) if
                                 is_duplicate_filename(filename, file)]
    ...
    @staticmethod
    def forward_files(friend: str, others: list[str], number: int = 10, is_maximize: bool = True,
                      close_wechat: bool = True, search_pages: int = 5, wechat_path: str = None) -> None:
        '''
        ...
        one_by_one_ForwardButton = \
        chat_history_window.descendants(**Texts.ForwardText)[0].parent().descendants(control_type='Button', title='')[0]
...
class WechatSettings():
    ...
    def Log_out(wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> None:
        ...
        Args:
            ...
            close_wechat:任务结束后是否关闭微信,默认关闭
        '''
        settings, main_window = Tools.open_settings(wechat_path=wechat_path, is_maximize=is_maximize,
                                                    close_wechat=close_wechat)
    ...
    @staticmethod
    def Auto_convert_voice_messages_to_text(state: Literal['close', 'open'], wechat_path: str = None,
                                            is_maximize: bool = True, close_wechat: bool = True,
                                            close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Adapt_to_PC_display_scalling(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                     close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Save_chat_history(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                          close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Run_wechat_when_pc_boots(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                 close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Using_default_browser(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                              close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Auto_update_wechat(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                           close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Close_auto_log_in(wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True,
                          close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Show_web_search_history(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def New_message_alert_sound(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Voice_and_video_calls_alert_sound(state: Literal['close', 'open'], wechat_path: str = None,
                                          is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Moments_notification_flag(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                  close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Channel_notification_flag(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                  close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Topstories_notification_flag(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                     close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Miniprogram_notification_flag(state: Literal['close', 'open'], wechat_path: str = None,
                                      is_maximize: bool = True, close_wechat: bool = True,
                                      close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Change_capture_screen_shortcut(shortcuts: list[str], wechat_path: str = None, is_maximize: bool = True,
                                       close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Change_open_wechat_shortcut(shortcuts: list[str], wechat_path: str = None, is_maximize: bool = True,
                                    close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Change_lock_wechat_shortcut(shortcuts: list[str], wechat_path: str = None, is_maximize: bool = True,
                                    close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    @staticmethod
    def Change_send_message_shortcut(shortcut: int = 0, wechat_path: str = None, is_maximize: bool = True,
                                     close_wechat: bool = True, close_settings_window: bool = True) -> None:
        '''
    ...
    def Change_Language(lang: int = 0, wechat_path: str = None, is_maximize: bool = True,
                        close_wechat: bool = True) -> None:
        '''
        该方法用来PC微信修改语言。
        Args:
    ...
    @staticmethod
    def Shortcut_default(wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True,
                         close_settings_window: bool = True) -> None:
        '''
...
class Call():
    ...
    @staticmethod
    def voice_call(friend: str, search_pages: int = 5, wechat_path=None, is_maximize: bool = True,
                   close_wechat: bool = True) -> None:
        '''
        该方法用来给好友拨打语音电话
        Args:
            ...
            close_wechat:任务结束后是否关闭微信,默认关闭
        '''
        main_window = Tools.open_dialog_window(friend, wechat_path, search_pages=search_pages, is_maximize=is_maximize)[
            1]
    ...
    @staticmethod
    def video_call(friend: str, search_pages: int = 5, wechat_path=None, is_maximize: bool = True,
                   close_wechat: bool = True) -> None:
        '''
        该方法用来给好友拨打视频电话
        Args:
    ...
    @staticmethod
    def voice_call_in_group(group_name: str, friends: list[str], search_pages: int = 5, wechat_path=None,
                            is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        该方法用来在群聊中发起语音电话
        Args:
        ...
        '''
        main_window = Tools.open_dialog_window(friend=group_name, search_pages=search_pages, wechat_path=wechat_path,
...
class FriendSettings():
    ...
    @staticmethod
    def pin_friend(friend: str, state: Literal['close', 'open'], search_pages: int = 5, wechat_path=None,
                   is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来将好友在会话内置顶或取消置顶
        Args:
        ...
        if state not in choices:
            raise WrongParameterError
        main_window, chat_window = Tools.open_dialog_window(friend=friend, wechat_path=wechat_path,
                                                            is_maximize=is_maximize, search_pages=search_pages)
    ...
    @staticmethod
    def mute_friend_notifications(friend: str, state: Literal['close', 'open'], search_pages: int = 5,
                                  wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来开启或关闭好友的消息免打扰
        Args:
        ...
        if state not in choices:
            raise WrongParameterError
        friend_settings_window, main_window = Tools.open_friend_settings(friend=friend, wechat_path=wechat_path,
                                                                         is_maximize=is_maximize,
                                                                         search_pages=search_pages)
    ...
    @staticmethod
    def sticky_friend_on_top(friend: str, state: Literal['close', 'open'], search_pages: int = 5,
                             wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来开启或关闭好友的聊天置顶
        Args:
    ...
    @staticmethod
    def clear_friend_chat_history(friend: str, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
    ...
    @staticmethod
    def delete_friend(friend: str, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                      close_wechat: bool = True):
        '''
        该方法用来删除好友
        Args:
            ...
            close_wechat:任务结束后是否关闭微信,默认关闭
        '''
        menu, friend_settings_window, main_window = Tools.open_friend_settings_menu(friend=friend,
    ...
    @staticmethod
    def add_new_friend(wechat_number: str, request_content: str = None, wechat_path: str = None,
                       is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        ...
        def send_request():
            ...
            if add_friend_request_window.exists():
                ...
                if request_content:
                    ...
                    pyautogui.hotkey('ctrl', 'a')
                    pyautogui.press('backspace')
                    request_content_edit = add_friend_request_window.child_window(title='', control_type='Edit',
                                                                                  found_index=0)
    ...
    @staticmethod
    def change_friend_remark(friend: str, remark: str, search_pages: int = 5, wechat_path: str = None,
                             is_maximize: bool = True, close_wechat: bool = True):
        '''
        该函数用来修改好友详情页面内的备注
        Args:
    ...
    @staticmethod
    def change_friend_tag(friend: str, tag: str, clear_all: bool = False, search_pages: int = 5,
                          wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来修改好友详情页的标签
        Args:
    ...
    @staticmethod
    def change_friend_description(friend: str, description: str, search_pages: int = 5, wechat_path: str = None,
                                  is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来修改好友详情页内的描述
        Args:
    ...
    @staticmethod
    def change_friend_phone_number(friend: str, phone_num: str, clear_all: bool = False, search_pages: int = 5,
                                   wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来修改好友详情页的电话号
        Args:
    ...
    @staticmethod
    def add_friend_to_blacklist(friend: str, state: Literal['close', 'open'], search_pages: int = 5,
                                wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来将好友添加至黑名单
        Args:
    ...
    @staticmethod
    def set_friend_as_star_friend(friend: str, state: Literal['close', 'open'], search_pages: int = 5,
                                  wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来将好友设置为星标朋友
        Args:
    ...
    @staticmethod
    def change_friend_privacy(friend: str, privacy: str, state: Literal['close', 'open'], search_pages: int = 5,
                              wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该函数用来修改好友权限
        Args:
    ...
    @staticmethod
    def share_contact(friend: str, others: list[str], search_pages: int = 5, wechat_path: str = None,
                      is_maximize: bool = True, close_wechat: bool = True):
        '''
        该函数用来推荐好友给其他人
        Args:
            ...
            for i in range(0, len(others), 9):  # 9个一批
                if i + 9 <= len(others):
                    for other_friend in others[i:i + 9]:
                        ...
                        pyautogui.press('backspace')
                    send_button.click_input()
                    profile_window = friend_settings_window.child_window(class_name="ContactProfileWnd",
                                                                         control_type="Pane", framework_id='Win32')
    ...
    @staticmethod
    def get_friend_wechat_number(friend: str, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                                 close_wechat: bool = True):
        '''
        该方法根据微信备注获取单个好友的微信号
        Args:
            ...
            close_wechat:任务结束后是否关闭微信,默认关闭
        '''
        profile_window, main_window = Tools.open_friend_profile(friend=friend, wechat_path=wechat_path,
                                                                is_maximize=is_maximize, search_pages=search_pages)
    ...
    @staticmethod
    def tickle_friend(friend: str, maxSearchNum: int = 50, search_pages: int = 5, wechat_path: str = None,
                      is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        该方法用来拍一拍好友
        Args:
        ...
        def find_firend_buttons_in_chatList(chatlist):
            if len(chatlist.children()) == 0:
                ...
                visible_buttons = [button for button in buttons if
                                   button.rectangle().bottom > top + 20 and button.rectangle().top < bottom - 20]  #
        ...
        def find_firend_chat_in_chat_history():
            ...
            if ChatMessage.exists():  # 文件传输助手或公众号没有右侧三个点的聊天信息按钮
                ...
                for _ in range(maxSearchNum):
                    ...
                    selected_item = \
                    [item for item in contentlist.children(control_type='ListItem') if item.is_selected()][0]
                    selected_items.append(selected_item)
    ...
    def get_chat_history(
        ...
        def ByNum():
            ...
            chat_history_window = \
            Tools.open_chat_history(friend=friend, wechat_path=wechat_path, is_maximize=is_maximize,
            ...
            for _ in range(number):
                pyautogui.press('up', _pause=False, presses=2)
                selected_item = [item for item in contentList.children(control_type='ListItem') if item.is_selected()][
                    0]
                selected_items.append(selected_item)
                #################################################
                # 当给定number大于实际聊天记录条数时
            ...
            if len(chat_history) < number:
                warn(message=f"你与{friend}的聊天记录不足{number},已为你导出全部的{len(chat_history)}条聊天记录",
                     category=ChatHistoryNotEnough)
            if is_json:
                ...
                if recent == 'Yesterday':
                    ...
                    while True:
                        ...
                        if '/' in parse_result[1]:
                            no_yesterday = True
                            break
                        if '/' not in parse_result[1] and ':' not in parse_result[1] and yesterday not in parse_result[
                            1]:
                            no_yesterday = True
                            break
                    if not no_yesterday:
...
class GroupSettings():
    ...
    @staticmethod
    def pin_group(group_name: str, state: Literal['close', 'open'], search_pages: int = 5, wechat_path=None,
                  is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来将群聊在会话内置顶或取消置顶
        Args:
        ...
        if state not in choices:
            raise WrongParameterError
        main_window, chat_window = Tools.open_dialog_window(friend=group_name, wechat_path=wechat_path,
                                                            is_maximize=is_maximize, search_pages=search_pages)
        Tool_bar = chat_window.child_window(found_index=1, title='', control_type='ToolBar')
        Pinbutton = Tool_bar.child_window(**Buttons.PinButton)
        if Pinbutton.exists():
    ...
    @staticmethod
    def create_group_chat(friends: list[str], group_name: str = None, wechat_path: str = None, is_maximize: bool = True,
                          messages: list[str] = [], delay: float = 0.4, close_wechat: bool = True) -> None:
        '''
        该函数用来新建群聊
        Args:
            friends:新群聊的好友备注列表。
    ...
    @staticmethod
    def change_group_name(group_name: str, change_name: str, search_pages: int = 5, wechat_path: str = None,
                          is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来修改群聊名称
        Args:
        ...
        group_chat_settings_window, main_window = Tools.open_group_settings(group_name=group_name,
                                                                            wechat_path=wechat_path,
                                                                            is_maximize=is_maximize,
                                                                            search_pages=search_pages)
    ...
    @staticmethod
    def change_my_alias_in_group(group_name: str, my_alias: str, search_pages: int = 5, wechat_path: str = None,
                                 is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来修改我在本群的昵称
        Args:
    ...
    @staticmethod
    def change_group_remark(group_name: str, remark: str, search_pages: int = 5, wechat_path: str = None,
                            is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来修改群聊备注
        Args:
    ...
    @staticmethod
    def show_group_members_nickname(group_name: str, state: Literal['close', 'open'], search_pages: int = 5,
                                    wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来开启或关闭显示群聊成员名称
        Args:
    ...
    @staticmethod
    def mute_group_notifications(group_name: str, state: Literal['close', 'open'], search_pages: int = 5,
                                 wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来开启或关闭群聊消息免打扰
        Args:
    ...
    @staticmethod
    def sticky_group_on_top(group_name: str, state='open', search_pages: int = 5, wechat_path: str = None,
                            is_maximize: bool = True, close_wechat: bool = True):
        '''
    ...
    @staticmethod
    def save_group_to_contacts(group_name: str, state: Literal['close', 'open'], search_pages: int = 5,
                               wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来将群聊保存或取消保存到通讯录
        Args:
    ...
    @staticmethod
    def clear_group_chat_history(group_name: str, search_pages: int = 5, wechat_path: str = None,
                                 is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来清空群聊聊天记录
        Args:
    ...
    @staticmethod
    def quit_group_chat(group_name: str, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                        close_wechat: bool = True):
        '''
        该方法用来退出微信群聊
        Args:
    ...
    @staticmethod
    def invite_others_to_group(group_name: str, friends: list[str], query: str = '拉好友进群', search_pages: int = 5,
                               wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        ...
        if showAllButton.exists():
            ...
            group_members_nicknames = [member.descendants(control_type='Button')[0].window_text() for member in
                                       group_members]
        else:
            group_members = [listitem for listitem in chatList.children(control_type='ListItem') if
                             listitem.window_text() not in Notfriends]
            group_members_nicknames = [member.descendants(control_type='Button')[0].window_text() for member in
                                       group_members]
        ...
        if friends:
            ...
            GroupInvitationApprovalWindow = main_window.child_window(
                **Windows.LargeGroupInvitationApprovalWindow)  # 群聊人数超过100以上拉人时必须点击确认发送邀请链接
            Add_member_window = main_window.child_window(**Main_window.AddMemberWindow)
    ...
    @staticmethod
    def remove_friends_from_group(group_name: str, friends: list[str], search_pages: int = 5, wechat_path: str = None,
                                  is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来将群成员移出群聊
        Args:
    ...
    @staticmethod
    def add_friends_from_group(group_name: str, friend: str, request_content: str = None, search_pages: int = 5,
                               wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来添加群成员为好友
        Args:
    ...
    @staticmethod
    def edit_group_notice(group_name: str, content: str, search_pages: int = 5, wechat_path: str = None,
                          is_maximize: bool = True, close_wechat: bool = True):
        '''
        该方法用来编辑群公告
        Args:
...
class Contacts():
    ...
    @staticmethod
    def get_friends_names(is_json: bool = False, wechat_path: str = None, is_maximize: bool = True,
                          close_wechat: bool = True) -> str:
        '''
        ...
        contacts_settings_window = \
        Tools.open_contacts_manage(wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=True)[0]
    ...
    @staticmethod
    def get_friends_info(is_json: bool = False, wechat_path: str = None, is_maximize: bool = True,
                         close_wechat: bool = True) -> str:
        '''
        ...
        def get_info():
            ...
            try:  # 通讯录界面右侧的好友信息面板
                ...
                try:
                    base_info_pane = \
                    main_window.children(title='', control_type='Pane')[1].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[1].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[0]
                except IndexError:
                    base_info_pane = \
                    main_window.children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[1].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[
                        0].children(title='', control_type='Pane')[0]
    ...
    @staticmethod
    def get_friends_detail(is_json: bool = False, wechat_path: str = None, is_maximize: bool = True,
                           close_wechat: bool = True) -> str:
        '''
        ...
        ##########################################
        # 转为json格式
        records = zip(nicknames, wechatnumbers, regions, remarks, phonenumbers, tags, descrptions, permissions,
                      common_group_nums, signatures, sources)
        contacts = [
            {'昵称': name[0], '微信号': name[1], '地区': name[2], '备注': name[3], '电话': name[4], '标签': name[5], '描述': name[6],
             '朋友权限': name[7], '共同群聊': name[8], '个性签名': name[9], '来源': name[10]} for name in records]
        if is_json:
    ...
    @staticmethod
    def get_groups_info(is_json: bool = False, wechat_path: str = None, is_maximize: bool = True,
                        close_wechat: bool = True) -> str:
        '''
        ...
        '''
        contacts_manage_pane = \
        Tools.open_contacts_manage(wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=close_wechat)[0]
        ...
        pyautogui.press('End')
        last_group_name = group_chatList.children(control_type='ListItem')[-1].descendants(control_type='Button')[
            0].window_text()
        pyautogui.press('Home')
        group_names = []
        group_numbers = []
    ...
    @staticmethod
    def get_group_members_info(group_name: str, is_json: bool = False, search_pages: int = 5, wechat_path=None,
                               is_maximize: bool = True, close_wechat: bool = True):
        '''
        ...
        if showAllButton.exists():
            ...
            showAllButton.click_input()
            group_members = [listitem for listitem in chatList.children(control_type='ListItem') if
                             listitem.descendants(control_type='Button')[0].window_text() != '']
            ...
            group_members_nicknames = [member.descendants(control_type='Button')[0].window_text() for member in
                                       group_members]
        info = {'群聊': group_name, '人数': len(group_members_alias), '群成员群昵称': group_members_alias,
                '群成员昵称': group_members_nicknames}
        if close_wechat:
            main_window.close()
        if is_json:
    ...
    @staticmethod
    def get_subscribed_oas(is_json: bool = False, wechat_path: str = None, is_maximize: bool = True,
                           close_wechat: bool = True) -> (list[dict] | str):
        '''
        该方法用来获取已关注的所有公众号的名称\n
        ...
        if not official_account.exists():
            ...
            while selected_item.window_text() != ListItems.OfficialAccountsListItem['title']:
                selected_item = \
                [item for item in ContactsLists.children(control_type='ListItem') if item.is_selected()][0]
                selected_items.append(selected_item)
                #################################################
                # 没必要继续向下了，此时已经到头了，可以提前break了
                # 也就是当前selected_item在selected_items的倒数第二个时，就可以直接退出了，当然，必须得保证selected_items大于2
...
class Moments():
    ...
    def dump_moments(wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        ...
        moments_window = \
        Tools.open_moments(wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=close_wechat)[0]
        moments_list = moments_window.child_window(**Lists.MomentsList)
        listitems = [listitem for listitem in moments_list.children(control_type='ListItem') if
                     listitem.window_text() != '']
        if not listitems:
        ...
        moments = []
        if scrollable:
            moments_list.iface_scroll.SetScrollPercent(verticalPercent=0.0,
                                                       horizontalPercent=1.0)  # 调用SetScrollPercent方法向上滚动,verticalPercent=0.0表示直接将scrollbar一下子置于顶部
            ...
            while current_pos < 0.99:
                ...
                if moments_list.children(control_type='ListItem')[-1].window_text() == '':
                    break
                moments.extend([Tools.parse_moments_content(listitem) for listitem in
                                moments_list.children(control_type='ListItem') if listitem.window_text() != ''])
        if not scrollable:
            moments.extend(
                [Tools.parse_moments_content(listitem) for listitem in moments_list.children(control_type='ListItem') if
                 listitem.window_text() != ''])
    ...
    @staticmethod
    def dump_recent_moments(recent: Literal['Today', 'Yesterday', 'Week', 'Month', 'Year'], wechat_path: str = None,
                            is_maximize: bool = True, close_wechat: bool = True):
        '''
        ...
        moments = []
        first_moment = \
        [ListItem for ListItem in moments_list.children(control_type='ListItem') if ListItem.window_text() != ''][0]
        first_moment_post_time = \
        first_moment.descendants(**Buttons.CommentButton)[0].parent().children(control_type='Text')[0].window_text()
        if year_sep in first_moment_post_time and recent == 'Year':
            ...
            if scrollable:
                ...
                while True:
                    ...
                    post_time = \
                    ListItems[0].descendants(**Buttons.CommentButton)[0].parent().children(control_type='Text')[
                        0].window_text()
    ...
    @staticmethod
    def export_recent_moments_images(recent: Literal['Today', 'Yesterday', 'Week', 'Month'] = 'Month',
        ...
        files = [os.path.join(sns_cache, filename) for filename in filenames if
                 not filename.endswith('_t') and not filename.endswith('_d')]
        # 使用filter继续进行筛选
    ...
    @staticmethod
    def export_recent_moments_videos(recent: Literal['Today', 'Yesterday', 'Week', 'Month'] = 'Month',
                                     target_folder: str = None, transcode: bool = False):
        '''
    ...
    def export_moments_cache(year: str = time.strftime('%Y'), month: str = None, target_folder: str = None):
        ...
        def classfiy(folder):
            ...
            # 按照修改时间对filenames排序
            files = [os.path.join(sns_cache, folder, file[0]) for file in
                     sorted(files, key=lambda x: x[1], reverse=True)]
            videos = list(filter(is_video, files))
            images = list(filter(is_image, files))
...
class AutoReply():
    ...
    @staticmethod
    def auto_answer_call(duration: str, broadcast_content: str, message: str = None, times: int = 1,
                         wechat_path: str = None, close_wechat: bool = True) -> None:
        '''
        该方法用来自动接听微信电话\n
            ...
            if call_interface1.exists():
                ...
                if flag == "语音通话":
                    ...
                    if accept_call_window.exists():
                        ...
                        answering_window = accept_call_window.child_window(found_index=13, control_type='Pane',
                                                                           title='')
                        if answering_window.exists():
                            reject = answering_window.child_window(**Buttons.HangUpButton)
                            reject.click_input()
                            if message:
                                Messages.send_message_to_friend(wechat_path=wechat_path, friend=caller_name,
                                                                close_wechat=close_wechat, message=message)
                else:
                    time.sleep(1)
                    accept.click_input()
                    time.sleep(1)
                ...
                if flag == "语音通话":
                    ...
                    if accept_call_window.exists():
                        ...
                        if answering_window.exists():
                            ...
                            if message:
                                Messages.send_message_to_friend(wechat_path=wechat_path, friend=caller_name,
                                                                message=message, close_wechat=close_wechat)
                else:
                    time.sleep(1)
                    accept.click_input()
                    time.sleep(1)
    ...
    @staticmethod
    def auto_reply_messages(content: str, duration: str, dontReplytoGroup: bool = False, max_pages: int = 5,
                            never_reply: list = [], scroll_delay: int = 0, wechat_path: str = None,
                            is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        ...
        def reply(names):
            names = [name for name in names if
                     name not in taboo_list]  # tabool_list是传入的never_reply和我这里定义的的一些不回复对象,比如'微信运动','微信支付'灯
            if names:
                for name in names:
    ...
    @staticmethod
    def auto_reply_to_friend(friend: str, duration: str, content: str, save_chat_history: bool = False,
                             capture_screen: bool = False, folder_path: str = None, search_pages: int = 5,
                             wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> (
            str | None):
        '''
        ...
        if count:
            if save_chat_history:
                chat_history = get_chat_history(friend=friend, number=2 * count, capture_screen=capture_screen,
                                                folder_path=folder_path, wechat_path=wechat_path,
                                                is_maximize=is_maximize, close_wechat=close_wechat)
                return chat_history
        Systemsettings.close_listening_mode()
        if close_wechat:
            main_window.close()
    ...
    @staticmethod
    def AI_auto_reply_to_friend(friend: str, duration: str, AI_engine, save_chat_history: bool = False,
                                capture_screen: bool = False, folder_path: str = None, search_pages: int = 5,
                                wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
        '''
    ...
    @staticmethod
    def auto_reply_to_group(group_name: str, duration: str, content: str, at_only: bool = True, maxReply: int = 3,
                            at_others: bool = True, save_chat_history: bool = False, capture_screen: bool = False,
                            folder_path: str = None, search_pages: int = 5, wechat_path: str = None,
                            is_maximize: bool = True, close_wechat: bool = True) -> (str | None):
        '''
        ...
        # 打开好友的对话框,返回值为编辑消息框和主界面
        Systemsettings.set_english_input()
        edit_area, main_window = Tools.open_dialog_window(friend=group_name, wechat_path=wechat_path,
                                                          is_maximize=is_maximize, search_pages=search_pages)
        ...
        mouse.click(coords=(x, y))
        responsed = []
        initialMessages = Tools.pull_messages(friend=group_name, number=maxReply, search_pages=search_pages,
                                              wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=False,
                                              parse=False)
        ...
        end_timestamp = time.time() + duration  # 根据秒数计算截止时间
        while time.time() < end_timestamp:
            newMessages = Tools.pull_messages(friend=group_name, number=maxReply, search_pages=search_pages,
                                              wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=False,
                                              parse=False)
            ...
            for newMessage in filtered_newMessages:
                message_sender, message_content, message_type = Tools.parse_message_content(ListItem=newMessage,
                                                                                            friendtype='群聊')
                send_message(message_content, message_sender)
                responsed.append(newMessage)
        if save_chat_history:
            chat_history = get_chat_history(friend=group_name, number=2 * len(responsed), capture_screen=capture_screen,
                                            folder_path=folder_path, wechat_path=wechat_path, is_maximize=is_maximize,
                                            close_wechat=close_wechat)
            return chat_history
        Systemsettings.close_listening_mode()
        if close_wechat:
    ...
    @staticmethod
    def auto_reply_messages_to_friends(content: str, duration: str, friends: list = [], max_pages: int = 5,
                                     scroll_delay: int = 0, wechat_path: str = None,
                                     is_maximize: bool = True, close_wechat: bool = True) -> None:
        '''
        该方法用来遍历会话列表查找新消息自动回复,最大回复数量=max_pages*(8~10)\n
        只会回复指定好友
        ...
        def listen_on_current_chat():
            ...
            if not video_call_button.exists() and not voice_call_button.exists():  # 好友
                type = '非好友'
            # 自动回复逻辑
            if type == '好友' and current_chat.window_text() in friends:
                latest_message, who = Tools.pull_latest_message(chatlist)  # 最新的消息
                if who == current_chat.window_text() and latest_message != initial_last_message:  # 不等于刚打开页面时的那条消息且发送者是对方
                    current_chat.click_input()
                    pyautogui.hotkey('ctrl', 'v', _pause=False)
...
def send_message_to_friend(friend: str, message: str, at: list[str] = [], at_all: bool = False, tickle: bool = False,
                           search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                           close_wechat: bool = True) -> None:
    '''
    该函数用于给单个好友或群聊发送单条信息
    ...
    if len(message) == 0:
        raise CantSendEmptyMessageError
    # 先使用open_dialog_window打开对话框
    edit_area, main_window = Tools.open_dialog_window(friend=friend, wechat_path=wechat_path, is_maximize=is_maximize,
                                                      search_pages=search_pages)
    chatContactMenu = main_window.child_window(**Panes.ChatContaceMenuPane)
    if is_maximize:
        main_window.maximize()
...
def send_message_to_friends(friends: list[str], message: list[str], tickle: list[bool] = [], delay: float = 0.2,
                            wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
...
def send_messages_to_friends(friends: list[str], messages: list[list[str]], tickle: list[bool] = [], delay: float = 0.4,
                             wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
    该函数用于给多个好友或群聊发送多条信息
    Args:
...
def forward_message(friends: list[str], message: str, delay: float = 0.4, search_pages: int = 5,
                    wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True):
    '''
...
def send_file_to_friend(friend: str, file_path: str, at: list[str] = [], at_all: bool = False,
                        with_messages: bool = False, messages: list = [], messages_first: bool = False,
                        delay: float = 1, tickle: bool = False, search_pages: int = 5, wechat_path: str = None,
                        is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
    该函数用于给单个好友或群聊发送单个文件
    Args:
...
def send_files_to_friend(friend: str, folder_path: str, with_messages: bool = False, messages: list = [str],
                         messages_first: bool = False, delay: float = 1, tickle: bool = False, wechat_path: str = None,
                         is_maximize: bool = True, close_wechat: bool = True, search_pages: int = 5) -> None:
    '''
    该函数用于给单个好友或群聊发送多个文件
    Args:
        friend:好友或群聊备注。格式:friend="好友或群聊备注"
...
def send_file_to_friends(friends: list[str], file_paths: list[str], with_messages: bool = False,
                         messages: list[list[str]] = [], messages_first: bool = False, delay: float = 1,
                         tickle: list[bool] = [], wechat_path: str = None, is_maximize: bool = True,
                         close_wechat: bool = True) -> None:
    '''
...
def send_files_to_friends(friends: list[str], folder_paths: list[str], with_messages: bool = False,
                          messages: list[list[str]] = [], message_first: bool = False, delay: float = 1,
                          tickle: list[bool] = [], wechat_path: str = None, is_maximize: bool = True,
                          close_wechat: bool = True) -> None:
    '''
...
def forward_file(friends: list[str], file_path: str, delay: float = 0.5, search_pages: int = 5, wechat_path: str = None,
                 is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
...
def Miniprogram_notification_flag(state: Literal['close', 'open'], wechat_path: str = None, is_maximize: bool = True,
                                  close_wechat: bool = True, close_settings_window: bool = True) -> None:
    '''
...
def pin_friend(friend: str, state: Literal['close', 'open'], search_pages: int = 5, wechat_path=None,
    ...
    if state not in choices:
        raise WrongParameterError
    main_window, chat_window = Tools.open_dialog_window(friend=friend, wechat_path=wechat_path, is_maximize=is_maximize,
                                                        search_pages=search_pages)
    Tool_bar = chat_window.child_window(found_index=1, title='', control_type='ToolBar')
    Pinbutton = Tool_bar.child_window(**Buttons.PinButton)
    if Pinbutton.exists():
...
def sticky_friend_on_top(friend: str, state: Literal['close', 'open'], search_pages: int = 5, wechat_path: str = None,
                         is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
    该函数用来开启或关闭好友的聊天置顶\n
    Args:
...
def delete_friend(friend: str, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
    ...
    Args:
        ...
        close_wechat:任务结束后是否关闭微信,默认关闭
    '''
    menu, friend_settings_window, main_window = Tools.open_friend_settings_menu(friend=friend, wechat_path=wechat_path,
                                                                                is_maximize=is_maximize,
                                                                                search_pages=search_pages)
    delete_friend_item = menu.child_window(**MenuItems.DeleteContactMenuItem)
    delete_friend_item.click_input()
...
def add_new_friend(wechat_number: str, request_content: str = None, wechat_path: str = None, is_maximize: bool = True,
                   close_wechat: bool = True) -> None:
    '''
...
def change_friend_tag(friend: str, tag: str, clear_all: bool = False, search_pages: int = 5, wechat_path: str = None,
                      is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
    该函数用来修改好友详情页的标签\n
    Args:
...
def change_group_name(group_name: str, change_name: str, search_pages: int = 5, wechat_path: str = None,
    ...
    if group_name == change_name:
        raise SameNameError(f'待修改的群名需与先前的群名不同才可修改！')
    group_chat_settings_window, main_window = Tools.open_group_settings(group_name=group_name, wechat_path=wechat_path,
                                                                        is_maximize=is_maximize,
                                                                        search_pages=search_pages)
    ChangeGroupNameWarnText = group_chat_settings_window.child_window(**Texts.ChangeGroupNameWarnText)
    if ChangeGroupNameWarnText.exists():
...
def clear_group_chat_history(group_name: str, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                             close_wechat: bool = True) -> None:
    '''
    该函数用来清空群聊聊天记录\n
    Args:
...
def add_friend_from_group(group_name: str, friend: str, request_content: str = None, search_pages: int = 5,
                          wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
    该函数用来添加群成员为好友\n
    Args:
...
def create_an_new_note(content: str = None, file: str = None, wechat_path: str = None, is_maximize: bool = True,
                       content_first: bool = True, close_wechat: bool = True) -> None:
    '''
    该函数用来创建一个新笔记\n
    Args:
        content:笔记文本内容
...
def auto_reply_to_friend(friend: str, duration: str, content: str, save_chat_history: bool = False,
                         capture_screen: bool = False, folder_path: str = None, search_pages: int = 5,
                         wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True) -> (str | None):
    '''
    ...
    def get_info():
        ...
        try:  # 通讯录界面右侧的好友信息面板
            ...
            try:
                base_info_pane = \
                main_window.children(title='', control_type='Pane')[1].children(title='', control_type='Pane')[
                    0].children(title='', control_type='Pane')[1].children(title='', control_type='Pane')[0].children(
                    title='', control_type='Pane')[0].children(title='', control_type='Pane')[0].children(title='',
                                                                                                          control_type='Pane')[
                    0].children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[0]
            except IndexError:
                base_info_pane = \
                main_window.children(title='', control_type='Pane')[0].children(title='', control_type='Pane')[
                    0].children(title='', control_type='Pane')[1].children(title='', control_type='Pane')[0].children(
...
def check_new_message(duration: str = None, save_file: bool = False, target_folder: str = None, wechat_path: str = None,
                      close_wechat: bool = True) -> list[dict]:
    '''
...
def listen_on_chat(friend: str, duration: str, save_file: bool = False, save_media: bool = False, save_method: int = 0,
                   file_folder: str = None, media_folder: str = None, search_pages: int = 5, wechat_path: str = None,
                   is_maximize: bool = True, close_wechat: bool = True) -> tuple[list[str], list[str], list[str]]:
    '''
    ...
    if not video_call_button.exists() and chat_history_button.exists():
        friendtype = '群聊'
    #####################################################################################
    initialMessages = Tools.pull_messages(friend=friend, number=5, search_pages=search_pages, wechat_path=wechat_path,
                                          is_maximize=is_maximize, close_wechat=False, parse=False)
    recorded.extend(initialMessages)
    Systemsettings.open_listening_mode(full_volume=False)
    end_timestamp = time.time() + duration  # 根据秒数计算截止时间
    while time.time() < end_timestamp:
        newMessages = Tools.pull_messages(friend=friend, number=5, search_pages=search_pages, wechat_path=wechat_path,
                                          is_maximize=is_maximize, close_wechat=False, parse=False)
    ...
    if save_media and photo_num:
        save_photos(friend=friend, number=photo_num, folder_path=media_folder, save_method=save_method,
                    is_maximize=is_maximize, search_pages=search_pages, close_wechat=close_wechat,
                    wechat_path=wechat_path)
        print(f'监听{unchanged_duration}内,已自动保存图片{photo_num}张')
    if save_media and video_num:
        save_videos(friend=friend, number=video_num, folder_path=media_folder, is_maximize=is_maximize,
                    search_pages=search_pages, close_wechat=close_wechat, wechat_path=wechat_path)
...
def save_files(friend: str, folder_path: str = None, number: int = 10, search_pages: int = 5, wechat_path: str = None,
               is_maximize: bool = True, close_wechat: bool = True) -> list[str]:
    '''
    ...
    if not folder_path:
        ...
        print(f'未传入文件夹路径,聊天文件将保存至 {folder_path}')
    # 打开好友的对话框,返回值为编辑消息框和主界面
    chat_history_window, main_window = Tools.open_chat_history(friend=friend, TabItem='文件', close_wechat=close_wechat,
                                                               wechat_path=wechat_path, is_maximize=is_maximize,
                                                               search_pages=search_pages)
    chat_file_folder = Tools.where_chatfiles_folder()
    fileList = chat_history_window.child_window(**Lists.FileList)
...
def at(group_name: str, group_member: str, is_send: bool = False, search_pages: int = 5, wechat_path: str = None,
       is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
...
def at_all(group_name: str, is_send: bool = False, search_pages: int = 5, wechat_path: str = None,
           is_maximize: bool = True, close_wechat: bool = True) -> None:
    '''
...
def auto_reply_to_group(group_name: str, duration: str, content: str, at_only: bool = True, maxReplynum: int = 3,
                        at_others: bool = True, save_chat_history: bool = False, capture_screen: bool = False,
                        folder_path: str = None, search_pages: int = 5, wechat_path: str = None,
                        is_maximize: bool = True, close_wechat: bool = True) -> (str | None):
    '''
    ...
    mouse.click(coords=(x, y))
    responsed = []
    initialMessages = Tools.pull_messages(friend=group_name, number=maxReplynum, search_pages=search_pages,
                                          wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=False,
                                          parse=False)
    responsed.extend(initialMessages)
    Systemsettings.copy_text_to_windowsclipboard(content)  # 复制回复内容到剪贴
    ...
    end_timestamp = time.time() + duration  # 根据秒数计算截止时间
    while time.time() < end_timestamp:
        newMessages = Tools.pull_messages(friend=group_name, number=maxReplynum, search_pages=search_pages,
                                          wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=False,
                                          parse=False)
        filtered_newMessages = [newMessage for newMessage in newMessages if newMessage not in responsed]
        for newMessage in filtered_newMessages:
...
def save_photos(friend: str, number: int, folder_path: str = None, save_method: int = 0, is_maximize: bool = True,
                close_wechat: bool = True, search_pages: int = 5, wechat_path: str = None) -> None:
    '''
    ...
    chat_history_window, main_window = Tools.open_chat_history(friend=friend, close_wechat=close_wechat,
                                                               search_pages=search_pages, is_maximize=is_maximize,
                                                               wechat_path=wechat_path)
    # 先激活scrollbar
    rec = chat_history_window.rectangle()
    mouse.click(coords=(rec.right - 10, rec.bottom - 10))
    ...
    if not image_preview_window.exists():
        chat_history_window, main_window = Tools.open_chat_history(friend=friend, TabItem='图片与视频',
                                                                   close_wechat=close_wechat, search_pages=search_pages,
                                                                   is_maximize=is_maximize, wechat_path=wechat_path)
    ...
    if save_method == 0:
        ...
        center = ((image_area.rectangle().right + image_area.rectangle().left) // 2,
                  (image_area.rectangle().bottom + image_area.rectangle().top) // 2)
...
def save_videos(friend: str, number: int, folder_path: str = None, is_maximize: bool = True, close_wechat: bool = True,
                search_pages: int = 5, wechat_path: str = None) -> None:
    '''
...
def forward_files(friend: str, others: list[str], number: int = 10, is_maximize: bool = True, close_wechat: bool = True,
                  search_pages: int = 5, wechat_path: str = None) -> None:
    '''
    该函数用来转发与某个好友或群聊聊天记录内的聊天文件给指定好友\n
    Args:
...
def forward_links(friend: str, others: list[str], number: int = 10, is_maximize: bool = True, close_wechat: bool = True,
                  search_pages: int = 5, wechat_path: str = None) -> None:
    '''
    ...
    Examples:
        ...
        ```
    '''
    chat_history_window, main_window = Tools.open_chat_history(friend=friend, TabItem='链接', wechat_path=wechat_path,
                                                               is_maximize=is_maximize, search_pages=search_pages,
                                                               close_wechat=close_wechat)
    rightClickMenu = chat_history_window.child_window(**Menus.RightClickMenu)
    selectMenuItem = rightClickMenu.child_window(**MenuItems.SelectMenuItem)
...
def forward_music_and_audio(friend: str, others: list[str], number: int = 10, is_maximize: bool = True,
                            close_wechat: bool = True, search_pages: int = 5, wechat_path: str = None) -> None:
    '''
    ...
    Examples:
        ...
        ```
    '''
    chat_history_window, main_window = Tools.open_chat_history(friend=friend, TabItem='音乐与音频', wechat_path=wechat_path,
                                                               is_maximize=is_maximize, search_pages=search_pages,
                                                               close_wechat=close_wechat)
    rightClickMenu = chat_history_window.child_window(**Menus.RightClickMenu)
    selectMenuItem = rightClickMenu.child_window(**MenuItems.SelectMenuItem)
...
def forward_MiniPrograms(friend: str, others: list[str], number: int = 10, is_maximize: bool = True,
                         close_wechat: bool = True, search_pages: int = 5, wechat_path: str = None) -> None:
    '''
    ...
    Examples:
        ...
        ```
    '''
    chat_history_window, main_window = Tools.open_chat_history(friend=friend, TabItem='小程序', wechat_path=wechat_path,
                                                               is_maximize=is_maximize, search_pages=search_pages,
                                                               close_wechat=close_wechat)
    rightClickMenu = chat_history_window.child_window(**Menus.RightClickMenu)
    selectMenuItem = rightClickMenu.child_window(**MenuItems.SelectMenuItem)
...
def export_videos(year: str = time.strftime('%Y'), month: str = None, target_folder: str = None) -> None:
    ...
    for folder_path in filtered_folders:  # 遍历筛选后的每个文件夹
        ...
        Videos = [os.path.join(folder_path, filename) for filename in os.listdir(folder_path) if
...
def get_subscribed_oas(is_json: bool = False, wechat_path: str = None, is_maximize: bool = True,
                       close_wechat: bool = True) -> (list[str] | str):
    '''
    该函数用来获取已关注的所有公众号的名称。\n
    结果以list[str]或该类型的json字符串返回\n
    Args:
...
def save_my_PaymentCode(folder_path: str, wechat_path: str = None, is_maximize: bool = True, close_wechat: bool = True,
                        close_ledger: bool = True):
    '''
    ...
    Systemsettings.copy_text_to_windowsclipboard(folder_path)
    payment_code_window = \
    Tools.open_payment_code_window(wechat_path=wechat_path, is_maximize=is_maximize, close_wechat=close_wechat,
                                   close_ledger=close_ledger)[0]
    save_paymentcode_text = payment_code_window.child_window(**Texts.SavePaymentCodeText).wait(wait_for='ready',
                                                                                               timeout=5)
    save_paymentcode_text.click_input()
    desktop = Desktop(**Independent_window.Desktop)
    save_as_window = desktop.window(
        **{'title_re': '另存为', 'control_type': 'Window', 'framework_id': 'Win32', 'top_level_only': False})
    while not save_as_window.exists():
        time.sleep(0.1)
    prograss_bar = save_as_window.child_window(control_type='ProgressBar', class_name='msctls_progress32',
                                               framework_id='Win32')
...
def listen_on_chats(friends: list[str], duration: str, is_json: bool = False, save_file: bool = False,
                    file_folder: str = None, search_pages: int = 5, wechat_path: str = None, is_maximize: bool = True,
                    close_wechat: bool = True) -> list[dict]:
    '''
    ...
    def listen_on_chat(chat, friend, friendtype):
        ...
        if save_file and filelist:
            ...
            print(
                f'监听至{time.strftime("%Y-%m-%d %H:%M:%S", time.localtime(end_timestamp))}结束,{friend}已自动保存文件{len(filelist)}个')
    ...
    # 同时打开多个好友的独立聊天窗口
    chats = Tools.open_dialog_windows(friends=friends, close_wechat=close_wechat, wechat_path=wechat_path,
                                      is_maximize=is_maximize)
    for chat in chats:
...
def dump_recent_session(recent: Literal['Today', 'Yesterday', 'Week', 'Month', 'Year'],
                        message_only: bool = False, no_official: bool = False, wechat_path: str = None,
                        is_maximize: bool = True, close_wechat: bool = True) -> tuple[list[str], list[str], list[str]]:
    '''
    ...
    if no_official:
        officialAccounts = get_subscribed_oas(is_json=False, wechat_path=wechat_path, is_maximize=is_maximize,
                                              close_wechat=False)
    ...
    if not scrollable:
        ...
        if no_official:
            ListItems = [ListItem for ListItem in ListItems if
                         ListItem.descendants(control_type='Text')[0].window_text() not in officialAccounts]
...
def dump_session_list(chatted_only: bool = False, no_official: bool = False, wechat_path: str = None,
                      is_maximize: bool = True, close_wechat: bool = True) -> tuple[list[str], list[str], list[str]]:
    '''
    ...
    if year_sep in first_moment_post_time and recent == 'Year':
        ...
        if scrollable:
            ...
            while True:
                ...
                ListItems = moments_list.children(control_type='ListItem')
                ListItems = [ListItem for ListItem in ListItems if ListItem.window_text() != '']
                post_time = ListItems[0].descendants(**Buttons.CommentButton)[0].parent().children(control_type='Text')[
                    0].window_text()
                # 往年时间戳是xxxx-xx-xx，时间戳里有-的都不是今年的
                if recent == 'Year' and year_sep in post_time:
                    break
...
def collections_reminder(duration: str, broadcast: bool = True, wechat_path: str = None, is_maximize: bool = True,
                         close_wechat: bool = True) -> tuple[list[str], list[str], list[str]]:
    '''
    ...
    def pull_messages(number):
        ...
        if len(chatList.children(control_type='ListItem')) == 0:  # 没有聊天记录直接返回空列表:
            return []
        ListItems = [message for message in chatList.children(control_type='ListItem') if
                     message.window_text() != Buttons.CheckMoreMessagesButton['title']]  # 产看更多消息内部也有按钮,所以需要筛选一下
        ...
        if len(ListItems) < number:  #
            ...
            if scrollable:
                ...
                while len(ListItems) < number:
                    chatList.iface_scroll.SetScrollPercent(verticalPercent=0.0,
                                                           horizontalPercent=1.0)  # 调用SetScrollPercent方法向上滚动,verticalPercent=0.0表示直接将scrollbar一下子置于顶部
                    mouse.scroll(coords=(x, y), wheel_dist=1000)
                    ListItems = [message for message in chatList.children(control_type='ListItem') if
                                 message.window_text() != Buttons.CheckMoreMessagesButton['title']]
                    ListItems = [message for message in ListItems if
                                 message.descendants(control_type='Text', title='个人收款服务')]
    ...
    try:
        desktop = Desktop(**Independent_window.Desktop)
        main_window = \
        Tools.open_dialog_window(friend='微信收款助手', wechat_path=wechat_path, is_maximize=is_maximize, search_pages=0)[1]
        message_list = main_window.child_window(**Main_window.ConversationList)
